...
// ── Default export so Vercel can import this file as its serverless handler ──
// Vercel looks for `export default` (or `module.exports`) from the entry file
// set in vercel.json. On local/Render the export is unused; the server.listen
// block below handles that path.
export default app;

// ── Local-dev / Render: start an HTTP server ─────────────────────────────────
// On Vercel, VERCEL=1 is always set automatically, so this block is skipped.
// connectDB() is NOT called here; app.js's per-request middleware handles it,
// keeping a single, consistent connection path for all deployment targets.
if (!process.env.VERCEL) {
    ...
    // On local/Render we eagerly connect so the first request isn't delayed by
    // the cold-start connection latency. The per-request middleware in app.js
    // returns the cached connection for all subsequent requests.
    const { default: connectDB } = await import("./db/databaseConnection.js");
